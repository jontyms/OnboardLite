...
def apple_wallet(user_data):
    ...
    with open(os.path.join(os.path.dirname(__file__), '..', 'static', 'apple_wallet', "icon.png"), 'rb') as file:
        ico_data = file.read()
        p.add_to_pass_package(("icon.png", ico_data))

    with open(os.path.join(os.path.dirname(__file__), '..', 'static', 'apple_wallet', "icon@2x.png"), 'rb') as file:
        ico_data = file.read()
    ...
    # Role-based logo.
    if is_ops:
        with open(os.path.join(os.path.dirname(__file__), '..', 'static', 'apple_wallet', "logo_ops@2x.png"), 'rb') as file:
            ico_data = file.read()
            p.add_to_pass_package(("logo@2x.png", ico_data))

        with open(os.path.join(os.path.dirname(__file__), '..',  'static', 'apple_wallet', "logo_ops.png"), 'rb') as file:
            ico_data = file.read()
            p.add_to_pass_package(("logo.png", ico_data))
    else:
        with open(os.path.join(os.path.dirname(__file__), '..',  'static', 'apple_wallet', "logo_reg@2x.png"), 'rb') as file:
            ico_data = file.read()
            p.add_to_pass_package(("logo@2x.png", ico_data))

        with open(os.path.join(os.path.dirname(__file__), '..',  'static', 'apple_wallet', "logo_reg.png"), 'rb') as file:
            ico_data = file.read()
            p.add_to_pass_package(("logo.png", ico_data))
...
async def aapl_gen(request: Request, token: Optional[str] = Cookie(None), payload: Optional[object] = {}):
    ...
    return Response(content=bytes(p), media_type="application/vnd.apple.pkpass", headers={
        "Content-Disposition": "attachment; filename=\"hackucf.pkpass\""    
    })